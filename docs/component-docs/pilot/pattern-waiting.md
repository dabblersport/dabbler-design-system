<!--
Pattern page, D-033(c)'s first of three. Not a component page — no
Definition/Axes/Tokens-used template applies; the page's job per D-033(c)
is to state the rule for choosing among components that already exist and
are already documented elsewhere in this set, never to repeat their pages.
...
Every claim traces to a page already written from a direct source read.
...
Four ways to say "not yet," and this system doesn't state which to reach for anywhere else — this
page is that rule.
...
[Spinner](spinner.md) · [ProgressBar](progress-bar.md) · [Skeleton](skeleton.md) ·
[Button](button.md)'s `loading` axis
